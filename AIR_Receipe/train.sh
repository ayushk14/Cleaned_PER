...
if [ ! -z "$feature_transform" ]; then
  ...
  case $feat_type in
    ...
    traps)
      ...
      python3 utils/nnet/gen_hamm_mat.py --fea-dim=$feat_dim --splice=$splice > $dir/hamm.mat
      python3 utils/nnet/gen_dct_mat.py --fea-dim=$feat_dim --splice=$splice --dct-basis=$traps_dct_basis > $dir/dct.mat
...
if [ ! -z $nnet_init ]; then
  ...
  case "$network_type" in
    dnn)
      python3 utils/nnet/make_nnet_proto.py $proto_opts \
    ...
    cnn1d)
      ...
      python3 utils/nnet/make_cnn_proto.py $cnn_proto_opts \
    ...
    cnn2d)
      delta_order=$([ -z $delta_opts ] && echo "0" || { echo $delta_opts | tr ' ' '\n' | grep "delta[-_]order" | sed 's:^.*=::'; })
      echo "Debug : $delta_opts, delta_order $delta_order"
      python3 utils/nnet/make_cnn2d_proto.py $cnn_proto_opts \
        --splice=$splice --delta-order=$delta_order --dir=$dir \
        $num_fea >$nnet_proto
      cnn_fea=$(cat $nnet_proto | grep -v '^$' | tail -n1 | awk '{ print $5; }')
      python3 utils/nnet/make_nnet_proto.py $proto_opts \
      ...
      ;;
    lstm)
      python3 utils/nnet/make_lstm_proto.py $proto_opts \
        $num_fea $num_tgt >$nnet_proto
      ;;
    blstm)
      python3 utils/nnet/make_blstm_proto.py $proto_opts \
        $num_fea $num_tgt >$nnet_proto
      ;;
